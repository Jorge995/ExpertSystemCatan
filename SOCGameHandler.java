    ...
    implements SOCGameEventListener
{
    ...
    private int contadorOfertas = 0;
    private int contadorConsejos = 0;
    ...
    public void sendTradeOffer(final SOCPlayer player, final Connection toJoiningClient)
    {
        ...
        ga.getAgent().getEnvironment().reset();
    	
    	//Datos jugador
    	int nplayer = ga.getAgent().getPlayerNumber();
    	SOCResourceSet resources = ga.getPlayer(nplayer).getResources();
    	int clay = resources.getAmount(SOCResourceConstants.CLAY);
    	int ore = resources.getAmount(SOCResourceConstants.ORE);
    	int wheat = resources.getAmount(SOCResourceConstants.WHEAT);
    	int sheep = resources.getAmount(SOCResourceConstants.SHEEP);
    	int wood = resources.getAmount(SOCResourceConstants.WOOD);
    	int usedknights = ga.getPlayer(nplayer).getNumKnights();
    	int usedroads = ga.getPlayer(nplayer).getRoadsAndShips().size();
    	int usedsettlements = ga.getPlayer(nplayer).getSettlements().size();
    	int usedcities = ga.getPlayer(nplayer).getCities().size();
    	int points = ga.getPlayer(nplayer).getTotalVP();
    	String turn = "";
    	if (ga.getCurrentPlayerNumber() == nplayer) {
    		turn = "yes";
    	}
    	else {
    		turn = "no";
    	...
    	int [] nodessettlementsposible = ga.getPlayer(nplayer).getPotentialSettlements_arr();
    	boolean citiesposible = ga.getPlayer(nplayer).hasPotentialCity();
    	String buildsettlementposible;
    	if (nodessettlementsposible != null) {
    		buildsettlementposible = "yes";
    	}
    	else {
    		buildsettlementposible = "no";
    	}
    	String objective = "";
    	if(buildsettlementposible == "yes") {
    		objective = "buildSettlement";
    	}
    	else {
    		if (!ga.getPlayer(nplayer).hasUnplayedDevCards()) {
    			objective = "getCard";
    		}
    		else {
    			if (usedroads < 6) {
    	...
    	ga.getAgent().getEnvironment().makeInstance("(of human (id "+nplayer+")(clay "+clay+")(ore "+ore+")(sheep "+sheep+")(wheat "+wheat+")(wood "+wood+")(nresources "+ga.getPlayer(nplayer).getResources().getKnownTotal()+")(used_knights "+usedknights+")(used_roads "+usedroads+")(used_settlements "+usedsettlements+")(used_cities "+usedcities+")(points "+points+")(turn "+turn+")(buildSettlementPosible "+buildsettlementposible+")(objective "+objective+"))");
    	...
    	for (int i = 0; i < players.length; i++) {
        	if (players[i].isRobot()) {
        		ga.getAgent().getEnvironment().makeInstance("(of enemy (id "+ players[i].getPlayerNumber() +")(points "+players[i].getTotalVP()+"))");
        	}
        }
    	
    	//Datos tablero
    	
    	int [] terrains = ga.getBoard().getLandHexCoords();
    	for (int i = 0; i<terrains.length; i++) {
        	String recurso = "";
        	int nrecurso = ga.getBoard().getHexTypeFromCoord(terrains[i]);
        	switch (nrecurso){
        		case 1:
        			recurso = "clay";
        			break;
        	...
        	int diceNumber = ga.getBoard().getNumberOnHexFromCoord(terrains[i]);
        	ga.getAgent().getEnvironment().makeInstance("(of terrain (id "+ terrains[i] +") (number "+ diceNumber +") (resource "+ recurso +"))");
    	}
    	
    	HashSet<Integer> nodes = ga.getBoard().nodesOnLand;
        for(int n : nodes){
            List<Integer> tierrasAdyacentes = ga.getBoard().getAdjacentHexesToNode(n);
            ga.getAgent().getEnvironment().makeInstance("(of vertex (id "+ n + ") (terrain1 "+ tierrasAdyacentes.get(0) +") (terrain2 "+ tierrasAdyacentes.get(1) +") (terrain3 "+ tierrasAdyacentes.get(2) + "))");
        }
    	
    	//Datos construcciones
        
        List<SOCSettlement> pueblos = ga.getPlayer(nplayer).getSettlements();
        for (int i = 0; i<pueblos.size(); i++) {
        	ga.getAgent().getEnvironment().makeInstance("(of settlement (vertex "+pueblos.get(i).getCoordinates()+"))");
        }
        
        List<SOCCity> ciudades = ga.getPlayer(nplayer).getCities();
        for (int i = 0; i<ciudades.size(); i++) {
        	ga.getAgent().getEnvironment().makeInstance("(of city (vertex "+ciudades.get(i).getCoordinates()+"))");
        }
        
        List<SOCRoutePiece> carreteras = ga.getPlayer(nplayer).getRoadsAndShips();
        for (int i = 0; i<carreteras.size(); i++) {
        	int edge = carreteras.get(i).getCoordinates();
        	List<Integer> nodos = ga.getBoard().getAdjacentNodesToEdge(edge);
        	ga.getAgent().getEnvironment().makeInstance("(of road (vertexFrom "+nodos.get(0)+")(vertexTo "+nodos.get(1)+"))");
        }
    	
    	//Datos oferta
    	int fromPlayer = offer.getFrom();
    	String ofertaString = "(of offer (fromPlayer "+ fromPlayer +") (toPlayer ";
    	boolean [] toPlayers = offer.getTo();
    	for (int i = 0; i < toPlayers.length; i++) {
    		if (toPlayers[i]) {
    			ofertaString = ofertaString + i +" ";
    		}
    	}
    	SOCResourceSet getResources = offer.getGetSet();
    	SOCResourceSet giveResources = offer.getGiveSet();
    	int getclay = getResources.getAmount(1);
    	int getore = getResources.getAmount(2);
    	int getsheep = getResources.getAmount(3);
    	int getwheat = getResources.getAmount(4);
    	int getwood = getResources.getAmount(5);
    	int giveclay = giveResources.getAmount(1);
    	int giveore = giveResources.getAmount(2);
    	int givesheep = giveResources.getAmount(3);
    	int givewheat = giveResources.getAmount(4);
    	int givewood = giveResources.getAmount(5);
    	
    	ofertaString = ofertaString + ") (getClay "+getclay+") (getOre "+getore+") (getSheep "+getsheep+") (getWheat "+getwheat+") (getWood "+getwood+") (giveClay "+giveclay+") (giveOre "+giveore+") (giveSheep "+givesheep+") (giveWheat "+givewheat+") (giveWood "+givewood+"))";
    	ga.getAgent().getEnvironment().makeInstance(ofertaString);
    	ga.getAgent().getEnvironment().run();
    	MultifieldValue mv = (MultifieldValue) ga.getAgent().getEnvironment().eval("(find-fact ((?f advice)) TRUE)");
    	if (offer.getTo()[nplayer]) {
    		this.contadorOfertas = this.contadorOfertas + 1;
    	}
    	...
    	if (mv.size() >= 1) {
    		...
    		String advice = "";
			try {
				advice = fv.getFactSlot("message").toString();
				
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
			this.contadorConsejos = this.contadorConsejos + 1;
			System.out.println(advice);
			System.out.println("Ofertas: "+ this.contadorOfertas);
			System.out.println("Consejos: "+ this.contadorConsejos);
    	}
    }
